...
class MainWindow(QMainWindow):
    def __init__(self, app, rppg: RPPG, winsize=(1000, 400), graphwin=150,
        ...
        # self.rppg.new_respr.connect(self.update_respr)
    ...
    def init_ui(self, winsize):
        ...
        self.setWindowTitle("rppg&respr prediction demo")
    ...
    def update_respr(self, respr):
        self.respr_label.setText("Respiratory rate: {:5.1f} breath/min".format(respr))
        
    def on_rppg_updated(self, results):
        ts = results.ts(self.graphwin)
        for pi, vs in enumerate(results.vs_iter(self.graphwin)):
            if len(vs) != len(ts):
                min_length = min(len(ts), len(vs))
                ts = ts[:min_length]
                vs = vs[:min_length]
